...
**所有商店类型（模板、插件、Agent Skills）共用此移动端模式。**

取消左侧导航栏，改为两个全屏页面之间的导航：
...
**移动端实现要点**：
- `@media (max-width: 767px)` 断点切换布局
- 列表页：隐藏 `.master-detail` 的 `.store-nav` 侧栏，卡片全宽单列排列
- 详情页：通过 CSS transform 或条件渲染实现页面推入/推出动画
- 返回按钮回到列表页，保持滚动位置
- 搜索栏和分类 chips 栏保持顶部固定
- Live Preview iframe 按视口宽度等比缩放（`transform: scale(viewportWidth / 1200)`）
- 此规范适用于 `StoreView.svelte` 通用组件，三种商店自动继承
...
`/templates` 路由，通过 iframe 嵌入 Presto 的 `/showcase/store-templates` 页面。商店 UI 由 Presto 仓库的 `StoreView.svelte` 渲染，官网仅提供 iframe 容器。

### 6.2 UI 设计

iframe 内部复用 Presto 的 `StoreView.svelte`，布局随 5.4 节的桌面端/移动端规范自动适配。官网不再维护独立的商店 UI 组件。
